...
/**
 * 使用说明：1.showLongTerm()是否显示长期选项<br/>
 * 2.setTimePickerGone隐藏时间选择<br/>
 * 3.接口DateChooseInterface<br/>
 * <p><br/>
 * 用于时间日期的选择<br/>
 * Created by liuhongxia on 2016/4/16.<br/>
 * ----------------
 * 从2015年开始选择<br/>
 * zhangzhenlin的意见：只能查当前某一天的告警<br/>
 * <br/>
 */
public class DateChooseWheelViewDialog extends Dialog implements View.OnClickListener {
    ...
    //变量
    private ArrayList<String> arry_year = new ArrayList<String>();   // 年
    private ArrayList<String> arry_date = new ArrayList<String>();  // 月日
    private ArrayList<String> arry_hour = new ArrayList<String>();  // 时
    private ArrayList<String> arry_minute = new ArrayList<String>(); // 分

    /**
     * 从今年开始向后倒数n年开始选择，<br/>
     * 比如，今年是2016年，那就是从2016-year_offset = 2014年开始选择
     */
    private int year_offset = 1;            // 默认从当前年的前一年开始选择
    private final int totalYearCount = 2;  // 供选择的年份数目，从上一年开始，一共两年：去年，今年
...
//                mDateCalendarTextView.setText(" " + arry_date.get(wheel.getCurrentItem()));
    ...
    private void initView() {
        ...
        mCloseDialog = (ImageButton) view.findViewById(R.id.date_choose_close_btn);
    ...
    private void initYear() {
        ...
        arry_year.clear();
        for (int i = 0; i <= totalYearCount - 1; i++) {
            int year = nowYear - year_offset + i;
...
//        }
    ...
    /**
     * xx年xx月xx日xx时xx分转成yyyy-MM-dd HH:mm<br/>
     * 之前是 2016-9-8 4:1，现转化为：2016-09-88 04:01:00
     *
     * @param yearStr
     * @param dateStr
     * @param hourStr
    ...
    private String strTimeToDateFormat(String yearStr, String dateStr, String hourStr, String minuteStr) {
        // 处理年
        yearStr = yearStr.replace("年", "-"); //2016年--> 2016-

        // 处理月日
        dateStr = dateStr.replace("月", "-").replace("日", "");
        String[] dateArray = dateStr.split("-");

        if (Integer.parseInt(dateArray[0]) < 10) { // 9 --> 09-
            dateArray[0] = "0" + dateArray[0] + "-";
        } else { // 10 --> 10-
            dateArray[0] = dateArray[0] + "-";
        }

        if (Integer.parseInt(dateArray[1]) < 10) { // 2 --> 02
            dateArray[1] = "0" + dateArray[1];
        }
        dateStr = dateArray[0] + dateArray[1]; // 09-02
        ...
        return yearStr + dateStr + " " + hourStr + ":" + minuteStr + ":00";
//        return yearStr.replace("年", "-") + dateStr.replace("月", "-").replace("日", " ") + hourStr + ":" + minuteStr;
    }
...
//    }
     ...
     * xx年xx月xx日xx时xx分转成yyyy-MM-dd HH:mm<br/>
     */
    public void setDefaultValue(String timeStr) {
        if (TextUtils.isEmpty(timeStr)) {
            return;
        }

        // todo 测试数据  -  弄好了
//        timeStr = "2016-11-01 01:09:23";
        try {
            String[] values = new String[5];
            String[] tmps1 = timeStr.split(" ");
            String[] tmps11 = tmps1[0].split("-");
            values[0] = tmps11[0];  // 年
            values[1] = tmps11[1];  // 月
            values[2] = tmps11[2];  // 日

            String[] tmps22 = tmps1[1].split(":");
            values[3] = tmps22[0];  // 时
            values[4] = tmps22[1];  // 分

            // 年  年也需要replace，不过目前没有0开头的年
            nowYearId = getIndexFromList(arry_year, values[0]+"年");
            mYearWheelView.setCurrentItem(nowYearId);
            ...
            if (values[2].startsWith("0")) {
                values[2] = values[2].replace("0","");
            }
            nowDateId = getIndexFromList(arry_date, values[1]+"月"+values[2]+"日");
            mDateWheelView.setCurrentItem(nowDateId);

            // 时
            if (values[3].startsWith("0")) {
                values[3] = values[3].replace("0","");
            }
            nowHourId = getIndexFromList(arry_hour, values[3]);
            mHourWheelView.setCurrentItem(nowHourId);

            // 分
            if (values[4].startsWith("0")) {
                values[4] = values[4].replace("0","");
            }
            nowMinuteId = getIndexFromList(arry_minute, values[4]);
            mMinuteWheelView.setCurrentItem(nowMinuteId);
    ...
    private int getIndexFromList(ArrayList<String> list, String value) {
